...
if __name__ == '__main__':
    ...
    extended_option = sys.argv[4].lower()  # used to test on 100K-60K data: for models trained on 60K
    extend_dataset = True if extended_option == 'true' else False
    ...
    dataset_path = os.path.expandvars(work_directory) + '/pred_denoising_test_data' + (
        '_extended' if extend_dataset else '') + ('_RES2' if residual2 else '') + ('_RES3' if residual3 else '')
    data_dict = dict()
    data_dict['pred'] = pred
