...
def extended_max_time_axis(signal):
    abs_signal = np.abs(signal)
    max_index = np.argmax(abs_signal, axis=2)
    print(max_index.shape)
    # print(signal[max_index].shape)
    exit(0)
    sign = np.sign(signal[max_index])
    extended_max = sign * abs_signal[max_index]
    return extended_max


def signal_to_noise_ratio_old_notworking(signal, noise, target):
    snr = []
    for i in range(signal.shape[0]):
        ...
        if valid_station_indices.shape[0] != 0:
            ...
            snr.append(10 * np.log10(np.sum(valid_signal ** 2) / np.sum(valid_noise ** 2)))
        else:
            snr.append(np.nan)
    return np.array(snr)
...
def load_mean_median_predictions(extended=False):
    ext_str = '_extended' if extended else ''
    data_dict = joblib.load('predictions/pred_denoising_test_data_mean_median_3_7_15' + ext_str)
    pred = data_dict['pred']
    return pred
...
def statistics_table(data, labels, predictions, save=False):
    ...
    for i, model in enumerate(models):
        table.add_row([model, f'{squared_errors[i].mean():.4f}', f'{absolute_errors[i].mean():.4f}'])
    print(table)

    table = PrettyTable(['Model', 'MSE ± std_se', 'MAE ± std_ae'])
    for i, model in enumerate(models):
        table.add_row([model, f'{squared_errors[i].mean():.4f} ± {squared_errors[i].std():.4f}',
                       f'{absolute_errors[i].mean():.4f} ± {absolute_errors[i].std():.4f}'])
    print(table)
...
def corr_coeff_vs_snr(data, labels, predictions):
    # for the moment, 0-dislocation samples are not taken into account
    snr = signal_to_noise_ratio(data, data - labels, labels)
    for i in range(len(predictions)):
        x, y = cc_as_function_of_snr(labels, predictions[i], snr, N_BINS=20)
...
def static_disp_vs_snr(data, labels, predictions):
    # for the moment, 0-dislocation samples are not taken into account
    snr = signal_to_noise_ratio(data, data - labels, labels)
    valid_snr_idx = snr > 0
    snr = snr[valid_snr_idx]
    for i in range(7, len(predictions)):
        x, y = static_disp_as_function_of_snr(labels[valid_snr_idx], predictions[i][valid_snr_idx], snr, N_BINS=20)
        plt.plot(x, y, label=models[i])
    plt.ylabel('Average relative error on static displacement')
    plt.xlabel('signal-to-noise ratio [dB]')
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    # print(deep_learning_predictions[-1][0,0,0,0])

    mean_median_predictions = load_mean_median_predictions(extended=extended_dataset)
    ...
    '''for n_samp in range(n_samples):
        for n_s in range(n_stations):
            for n_c in range(n_components):
                data[n_samp, n_s, :, n_c] = data[n_samp, n_s, :, n_c] - np.mean(data[n_samp, n_s, :, n_c])'''

    # Calculate the mean along the time axis (axis=2) for each component
    '''mean_vals = np.mean(X, axis=2, keepdims=True)

    # Remove the mean from each component
    time_series_data_centered = X - mean_vals
    ...
    for i in range(100):
        ...
        a1, a2 = a
        a1.plot(X[i, :, :, 0].T)
        a2.plot(time_series_data_centered[i, :, :, 0].T)
        plt.show()'''
    ...
    err_vs_disp(y, predictions)
    exit(0)

    # corr_coeff_vs_snr(X, y, predictions)
    # static_disp_vs_snr(X, y, predictions)

    # err_vs_snr(X, y, predictions)

    ### error_vs_snr(X, y, predictions)  # not correct
