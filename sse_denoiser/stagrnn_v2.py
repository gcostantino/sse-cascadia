"""Cascade of 3 graph-recurrent units"""
...
from tsl.nn.blocks.encoders import AGCRN, AGCRNCell
...
class AGCRNSingleAdjacency(RNNBase):
    ...
    def __init__(self, input_size: list, emb_size: int, hidden_size: list,
                 num_nodes: int,
        ...
        super(AGCRNSingleAdjacency, self).__init__(rnn_cells, cat_states_layers,
    ...
    def forward(self, x: Tensor, h: Optional[Tensor] = None):
        ...
        return super(AGCRNSingleAdjacency, self).forward(x, h=h, adj=adj, e=emb)


class STAGRNNDenoiserModelCascade(torch.nn.Module):
    def __init__(self, residual=False, learn_static=False, return_attention=False, residual2=False, residual3=False):
        super(STAGRNNDenoiserModelCascade, self).__init__()
        # torch.manual_seed(12345)
        self.n_components = 2
        self.transformer_dropout = 0.1
        self.adj_embedding_dim = 16
        self.add_transformer = True
        ...
        self.single_adjacency = True

        rnn_input = self.n_components

        if self.single_adjacency:
            input_dim = [rnn_input, 32, 64]
            hidden_dim = [32, 64, 128]
            self.agcrn = AGCRNSingleAdjacency(input_size=input_dim, emb_size=self.adj_embedding_dim,
                                hidden_size=hidden_dim, num_nodes=self.n_nodes,
                                n_layers=len(hidden_dim))
        else:
            self.agcrn1 = AGCRN(input_size=rnn_input, emb_size=self.adj_embedding_dim,
                               hidden_size=32, num_nodes=self.n_nodes,
                               n_layers=self.n_recurrent_layers)
            self.agcrn2 = AGCRN(input_size=32, emb_size=self.adj_embedding_dim,
                               hidden_size=64, num_nodes=self.n_nodes,
                               n_layers=self.n_recurrent_layers)
            self.agcrn3 = AGCRN(input_size=64, emb_size=self.adj_embedding_dim,
        ...
        if self.add_transformer:
            self.transformer = SpatioTemporalTransformer(input_size=self.rnn_hid_size, hidden_size=self.rnn_hid_size,
                                                         axis='both', causal=False, dropout=self.transformer_dropout,
    ...
    def forward(self, x_in, visualize_rnn_feat=False, visualize_transf_feat=False):
        ...
        if self.single_adjacency:
            ...
            x_out1, h = self.agcrn1(x_perm)
            x_out2, h = self.agcrn2(x_out1)
            x_out, h = self.agcrn3(x_out2)
...
class STAGRNNDenoiserCascade:
    ...
    def build(self):
        ...
        model = STAGRNNDenoiserModelCascade(residual=self.residual, learn_static=self.learn_static,
    ...
    def ang_loss_slow(self, ts_true, ts_pred, disp_true, disp_pred):
        ...
        for i in range(disp_true.size(0)):
            if torch.sum(disp_true[i]) != 0:
                angular_misfit = torch.mean(torch.arccos(torch.clamp(disp_loss(disp_true[i], disp_pred[i]), -1.0 + 1e-07, 1.0 - 1e-07)))
