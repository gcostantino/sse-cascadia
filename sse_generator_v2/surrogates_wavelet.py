...
class SurrogatesWavelet:
    ...
    def correlated_noise_surrogates(self, original_data, wavelet):
        """
        ...
        **Examples:**
        ...
        if self._fft_cached:
            ...
            surrogates = pywt.wavedec(original_data, wavelet, axis=1)  # np.fft.rfft(original_data, axis=1)
        ...
        return np.ascontiguousarray(np.real(pywt.waverec(surrogates, wavelet, axis=1)))

    def AAFT_surrogates(self, original_data, wavelet):
        """
        ...
        phase_randomized_data = \
            self.correlated_noise_surrogates(rescaled_data, wavelet)
    ...
    def refined_AAFT_surrogates(self, original_data, n_iterations,
                                output="true_amplitudes", wavelet='db1'):
        """
        ...
        if self._fft_cached:
            wavelet_transform = self._original_data_fft
        else:
            wavelet_transform = pywt.wavedec(original_data, wavelet, axis=1, level=1)  # np.fft.rfft(original_data, axis=1)
            wavelet_transform = np.array(wavelet_transform)
            self._original_data_fft = wavelet_transform
            self._fft_cached = True

        #  Get Fourier amplitudes
        original_fourier_amps = np.abs(wavelet_transform)
        ...
        # (see [Schreiber2000]_)
        R = self.AAFT_surrogates(original_data, wavelet)

        #  Start iteration
        for i in range(n_iterations):
            #  Get Fourier phases of R surrogate
            r_fft = pywt.wavedec(R, wavelet, axis=1)  # np.fft.rfft(R, axis=1)
            ...
            # s = np.fft.irfft(original_fourier_amps * r_phases, n=n_time, axis=1)
            s = pywt.waverec(original_fourier_amps * r_phases, wavelet, axis=1)
